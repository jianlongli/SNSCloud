...
<?php echo $this->tag->form(array('notice/save/' . $notice_id, 'id' => 'registerForm', 'onbeforesubmit' => 'return false', 'enctype' => 'multipart/form-data')); ?>	                
                                <div class="Qx1">
                                    <div class="Qx11">通知名称：</div><input type="hidden" name="notice_id" value="<?php echo $notice_id; ?>">
                                    <input type="hidden" name="circle_id" value="<?php echo $circle_id; ?>">
                                    <div class="Qx12"><input type="text" class="inputNam" id="notice_title" name="notice_title" placeholder="标题" value="<?php echo $notice->notice_title; ?>" required /></div>
                                    <div class="clear"></div>
                                </div>
                                <div class="Qx1">
                                    <div class="Qx11">接收人：</div>
                                    <div class="Qx12">
                                    	<input type="radio" name="receive" <?php if ($notice->receive_people == 0) { ?> checked <?php } ?> value="0" />圈内所有人　
                                        <input type="radio" name="receive" <?php if ($notice->receive_people == 1) { ?> checked <?php } ?> value="1"/>指定人　
                                         <input type="text" class="inputNam" name="receive_people" id="receive_people" value=<?php if ($notice->receive_people == 1) { ?> "<?php echo $people; ?>" <?php } ?>/>
                                 ...
                                 <div class="Qx1">
                                    ...
                                    <div class="Qx12">
                                    	<input type="radio" name ="back" value="1"<?php if ($notice->back == 1) { ?> checked <?php } ?>/>是　
                                        <input type="radio" name="back" value="0" <?php if ($notice->back == 0) { ?> checked <?php } ?>/>否　
                                     </div>
                                    <div class="clear"></div>
                                </div>
                                <div class="Qx1">
                                    <div class="Qx11">通知内容：</div>
                                    <div class="Qx12"><textarea name="content" class="textarea" placeholder="请填写通知内容" required><?php echo $notice->content; ?></textarea></div>
                                    <div class="clear"></div>
                                </div>
                                <div class="Qx1">
                                    <div class="Qx11">通知附件：</div>
                                    <div class="Qx12">
                                       <input type="file" name="notice_fujian" value=""/><?php echo $notice->fujian_name; ?>
