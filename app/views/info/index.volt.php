...
<?php echo $this->tag->javascriptInclude('./eduis/js/circleinfo.js'); ?>
...
<script>
	...
	$(function(){
			...
			var _filterId = $('#existUserid').val();
				...
				ok:function() {
					$.each(this.iframe.contentWindow.$("input[data-name]:checked"), function(i, n){
						var _id = $(n).val();
						$.circleinfo._setData( '/info/membermanage' ,{id:_circleId, type: 'add', memberId:_id} , memberManageCallback );
					});
				},
				cancel: true
			});
		...
		$(".syspage").live('click',function (){
			...
			if($(".current").data("name") == 'member'){
				...
				if(_key.length > 0 )
					_param.key = _key;
				$.circleinfo._getData( _url , $(".current").data("name"), _param );
			}
		});
		...
		$(".memberManage").live('click' , function(){
			$.circleinfo._setData( '/info/membermanage' ,{id: $(this).data('id') , type: $(this).data('type')} , memberManageCallback );
		});
	...
	function memberManageCallback( _data ){
		$.circleinfo._init($('li[class="current"]').data('name'),{id:_circleId});
	}
