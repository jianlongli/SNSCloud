(function ($) {
    ...
    $.alertInfo = function (topInfo, cengerInfo, centerimg, width, height) {
        ...
        $("<div class='" + divs + "'><div class='divsalerttop' onMouseDown='$.alertdow(event);' onMouseUp='$.alertdup(event);'>" + topInfo + "<span><a href='#this' onclick='$.alertdel();'>关闭</a></span></div><div class='divsalertcenter'><div class='divsalertcenter_left'><img src=" + centerimg + " alt='我是提示图片'/></div><div class='divsalertcenter_right'>" + cengerInfo + "</div></div></div>").appendTo(info);
        alertwidth = width;
        alertheight = height;
        alertcss();
    }
    ...
    function alertcss() {
        winmx();
        var str = "";
        str += ".hi" + divs + "{opacity:0.6;filter:alpha(opacity=60);background-color:#ccc; position:absolute;top:0px;left:0px;z-index:200;width:" + winwid + "px;height:" + winhei + "px;}"; //幕布
        ...
        str += "." + divs + "{font-size:13px; border:1px solid #ccc;width:" + alertwidth + "px;z-index:201;height:" + alertheight + "px;position:absolute;left:" + ((winwid - alertwidth) / 2) + "px;top:" + (((document.documentElement.clientHeight - alertheight) / 2)) + "px;background-color:#fff;}";
    ...
    $.alertUrl = function (url, topInfo, width, height) {
        ...
        $("<div class='hi" + divs + "'></div>").appendTo(info);
	var sc="<div class='" + divs + "'><div class='divsalerttop' onMouseDown='$.alertdow(event);' onMouseUp='$.alertdup(event);'>" + topInfo + "<span><a href='#this' onclick='$.alertdel();'  style=' text-decoration:none; color:#ce1c1c;'>关闭</a></span></div><div class='divsalertcenter'> <iframe frameborder='no' src='" + url + "'></iframe></div></div>";
$("body").html($("body").html()+sc);
        alertwidth = width;
        alertheight = height;
        alerturlcss();
    }
    ...
    function alerturlcss() {
        winmx();
        var str = "";
        str += ".hi" + divs + "{opacity:0.5;filter:alpha(opacity=50);background-color:#ccc; position:absolute;top:0px;left:0px;z-index:200;width:" + winwid + "px;height:" + winhei + "px;}"; //幕布
        //str += "." + divs + "{ border:1px solid #ccc;width:" + alertwidth + "px;z-index:201;height:" + alertheight + "px;position:absolute;left:" + ((winwid - alertwidth) / 2) + "px;top:" + (document.documentElement.scrollTop + ((document.documentElement.clientHeight - alertheight) / 2)) + "px;background-color:#fff;}";
        str += "." + divs + "{ font-size:16px; font-weight:bold; border:1px solid #ccc;width:" + alertwidth + "px;z-index:201;height:" + alertheight + "px;position:absolute;left:" + ((winwid - alertwidth) / 2) + "px;top:" + (((document.documentElement.clientHeight - alertheight) / 2)) + "px;background-color:#fff;}";
        str += "." + divs + "{ position:fixed; cursor:pointer;_position:absolute;_bottom:auto;_top:expression(eval(document.documentElement.scrollTop+document.documentElement.clientHeight-this.offsetHeight-(parseInt(this.currentStyle.marginTop,10)||0)-(parseInt(this.currentStyle.marginBottom,10)||0)));}";

        str += ".divsalerttop{border-bottom:1px solid #f3c1c1; background-color:#fdf5f5;width:" + alertwidth + "px;height:22px;padding-top:8px;cursor:move;}";
        str += ".divsalerttop span{position:absolute;right:10px;}";
        str += ".divsalertcenter{width:" + alertwidth + "px;height:" + (alertheight - 30) + "px;}";
