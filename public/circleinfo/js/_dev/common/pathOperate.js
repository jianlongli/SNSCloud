define(function(require, exports) {
	...
	var _json = function(json){
		...
		var send = 'list=[';
		for (var i in json) {
			send += '{"type":"'+json[i].type+'","path":"'+json[i].path+'"}';
	...
	var newFile = function(path,callback){
		...
		$.ajax({
			dataType:'json',
			url: '/circle/mkfile?path='+path+'&filename='+filename,
			beforeSend:function(){
				core.tips.loading();
			},
	...
	var newFolder = function(path,callback){
		...
		$.ajax({
			dataType:'json',
			url: '/circle/mkdir?path='+path+'&filename='+filename,
			beforeSend:function(){
				core.tips.loading();
			},
			error:core.ajaxError,
	...
	var rname = function(from,to,callback){
		...
		$.ajax({
			type: "POST", 
			dataType:'json',
			url: '/circle/pathRname',
			data: '&from='+from+'&rname_to='+to,
			beforeSend:function(){
				core.tips.loading();
			},
			error:core.ajaxError,
		...
		$.dialog({
			...
			ok:function() {
				$.ajax({
					...
					success: function(data) {
						core.tips.close(data);
						var clickName = window.location.hash;
						if (clickName.length > 0) {
							clickName = clickName.substring(1, clickName.length);
							document.getElementById(clickName).click();
						}
						else {
							if (typeof(callback) == 'function')callback(data);// Todo:修改该方法控制跳转目录
						}
	...
	var past = function(path,callback){
		if (!path) return;
		var url='/circle/pathPast/?c_path=' + urlEncode2(path);
		$.ajax({
			url:url,
			dataType:'json',
			beforeSend: function(){
	...
	var cuteDrag = function(param,dragTo,callback){
		if (!dragTo) return;
		$.ajax({
			url:'index/pathCuteDrag',
			type:'POST',
			dataType:'json',
			data:_json(param)+'&path='+dragTo,
			beforeSend: function(){
				core.tips.loading(LNG.moving);
			},
			error:core.ajaxError,
