...
//				return host+'index/fileProxy&path=' +urlEncode(path);
				return '/circle/fileProxy?path=' +urlEncode(path);
			}else{
//				return G.web_host+G.web_root+path;
		...
		setting:function(setting){
			...
			if (window.top.frames["Opensetting_mode"] == undefined) {
				$.dialog.open('/index/setting#'+setting,{
					id:'setting_mode',
					fixed:true,
		...
		upload:function() {
			...
			uploader.option('server','/circle/fileUpload?path='+G.upload_path);
