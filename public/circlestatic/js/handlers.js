...
//			alert("您正在上传的文件队列过多.\n" + (message === 0 ? "您已达到上传限制" : "您最多能选择 " + (message > 1 ? "上传 " + message + " 文件." : "一个文件.")));
			 ...
			 * 2014/09/03
			 */
			if ($('#uploadHeaderDiv').size()) {
				$('#uploadHeaderDiv').removeClass('success').addClass('attention');
				$('#uploadError').html('请选择一个照片进行上传');
				$('#uploadHeaderDiv').slideDown(500);
				setTimeout(function(){
					if ($('#uploadHeaderDiv').css('display') == 'block') {
						$('#uploadHeaderDiv').slideUp(500);
					}
				}, 3000);
			}
			return;
