...
mSift.prototype={
	...
	Get:function(){
		...
		for(var i=0;i<_this.ReData.length;i++){
			...
			cResult+='<li style="cursor:default;width:290px;border:none;height:25px;   text-align:left;" onmouseover="'+
			_this.oo+'.ChangeOn(this);'+_this.oo+'.SelIndex='+i+';" onmousedown="'+_this.oo+'.Select();">'
			+_this.ReData[i].replace(cRegEx,function(s){return '<span style="background:#ff9;height:25px;font-weight:bold;font-style:normal;color:#a3253b; text-align:left;">'+s+'</span>';});+'</li>';
		}
		if(cResult==''){_this.Clear();}
		else{
			_this.TgList.innerHTML=cResult;
			_this.TgList.style.cssText='display:block;position:absolute;background:#fff;border:#999 solid 1px;margin:-1px 0 0;padding:0px 5px;list-style:none;font-size:14px; text-align:left;';
		...
		if(oLi.length>0){
			oLi[_this.SelIndex].style.cssText='background:#f5d5d8;border:none;height:25px; width:290px;;cursor:default;color:#a3253b; text-align:left;';
		}
	},
	ChangeOn:function(oObj){
		var oLi=this.TgList.getElementsByTagName('li');
		for(var i=0;i<oLi.length;i++) {
			oLi[i].style.cssText='width:290px;cursor:default; text-align:left;border:none;height:25px;';
		}
		oObj.style.cssText='background:#f5d5d8;width:290px;cursor:default;color:#a3253b; text-align:left;border:none;height:25px;';
	},
	Clear:function(){
		var _this=this;
		if(_this.TgList){
