...
//		}
	 ...
	 * 首次加载列表
	 */
	var _setCircleList = function ( _data ) {
		if (_data.code) {
			var _circle_data = _data.data.items;
			var _html = "";
			$.each(_circle_data, function(i,n){
				if (i === 0) {
					_html += "<div class='qGirleftx'>";
				}
				_html += "<div class='qGIrleftx_x'><ul>";
				_html += "<li><a href='#'><img src='" + n.logoUrl + "' width='176' height='176' /></a><div class='hudiv'>圈主：<span>" + n.userName + "</span></div></li>";
				_html += "<li><a href='#'>" + n.name + "</a></li>";
				_html += "<li>成员：<span>36</span>　资源：<span>36</span>　讨论：<span>36</span></li>";
				_html += "<li>作业：<span>36</span>　通知：<span>36</span></li>";
				_html += "</ul></div>";
				if (i%7 === 0 && i != 0) {
					_html += "<div class='clear'></div></div><div class='qGirleftx'>";
				}
			});
...
//			.html(eval(G.sort_order));
	...
	this._getFolderBox = function(list){
		var html="";
		html+="<div class='file folderBox menufolder' path='"+list.path+"' data-name='"+list.name+"' title='"
	...
	this._getFileBox = function(list){
		...
		if (list['ext'] == 'oexe' && list['icon'] != undefined) {
			...
			html ="<div class='file fileApp menuApp' path='"+list.path+"' data-app="+code+" data-name='"+list.name+"' title='"
			...
			var thumbPath = 'index/image?path='+urlEncode(G.this_path+list['name']);
//			alert(thumbPath);
			html+="<div class='file fileBox menufile' path='"+list.path+"' data-name='"+list.name+"' title='"
			+LNG.name+':'+list.name+"&#10;"+LNG.size+':'+list.size_friendly+"&#10;"
			+LNG.modify_time+':'+list.mtime+"'>";
	...
	var _f5 = function(is_data_server,is_animate,callback) {
		...
		if(!is_data_server){//采用当前数据刷新,用于显示模式更换
			...
			$.ajax({
				url : 'circle/getlist?circle_id=' + G.circle_id + '&page=' + G.page_id,
				dataType:'json',
				beforeSend:function(){
...
//		picasa:MyPicasa,
		...
		init:function(){
			_f5_callback(function( _data ){//数据首次加载后回调
				//_setListType(G.list_type,true);
				_setCircleList ( _data );
			});
			//生成文件列表
			//_bindEventSort();
			//_bindEventTheme();
